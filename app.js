...
// wheel: ctrl/meta to zoom, otherwise pan
...
wrapper.addEventListener('wheel', (e)=>{
  ...
  if(e.ctrlKey || e.metaKey){
    ...
    tx = tx + mx*(1/newScale - 1/scale);
    ty = ty + my*(1/newScale - 1/scale);
    scale = newScale;
    applyViewportTransform();
    // redraw links to account for new transform
    drawLinks();
  } else {
    // pan
    tx -= e.deltaX;
...
function recenter(){
  // reset scale
  scale = 1; tx = 0; ty = 0;
  // compute bounding box of displayed node elements (content coords using offsetLeft)
  const nodesEls = Array.from(document.querySelectorAll('.node-box'));
  if(nodesEls.length===0){ applyViewportTransform(); return; }
  let minL = Infinity, maxR = -Infinity, minT = Infinity;
  nodesEls.forEach(el=>{
    const l = el.offsetLeft; const r = l + el.offsetWidth; const t = el.offsetTop;
    if(l < minL) minL = l; if(r > maxR) maxR = r; if(t < minT) minT = t;
  });
  const contentW = maxR - minL;
  const wrapperW = wrapper.clientWidth;
  // center horizontally
  tx = Math.max(0, (wrapperW - contentW)/2) - minL;
  // small top padding
  ty = 20 - minT;
  applyViewportTransform(); drawLinks();
}
